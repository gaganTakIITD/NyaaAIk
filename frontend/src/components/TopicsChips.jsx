// Clean text labels only — pre-fills input so user can edit before sending
const TOPICS = [
  { label: 'Theft & Robbery', query: 'What are the legal provisions for theft under Bharatiya Nyaya Sanhita 2023 and what are the landmark cases?' },
  ...
  { label: 'Consumer Complaints', query: 'How do I file a consumer complaint in India for defective goods or poor service?' },
  { label: 'Property Dispute', query: 'What documents should I verify before buying residential property in India?' },
...
export default function TopicsChips({ onPrefill, disabled }) {
  return (
    <div className="topics-row" role="list" aria-label="Quick topic suggestions">
      {TOPICS.map((t, i) => (
        <button
          key={i}
          className="topic-chip"
          onClick={() => !disabled && onPrefill(t.query)}
          disabled={disabled}
          title="Click to pre-fill — you can edit before sending"
          role="listitem"
        >
          {t.label}
        </button>
