...
2. In the **first code cell**, set **`REPO_ROOT`** to your Repos checkout (Workspace sidebar → right-click the repo → **Copy path**). That cell installs **NumPy 2.x**, **`faiss-cpu`**, and related RAG deps into the notebook kernel, then `pip install -e ...[rag,rag_embed]`. Recent **faiss-cpu** expects **`numpy._core`** (NumPy 2+); if you see `No module named 'numpy._core'`, re-run the first cell so pip upgrades NumPy before importing faiss.
