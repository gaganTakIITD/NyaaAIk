...
2. In the **first code cell**, set **`REPO_ROOT`** to your Repos checkout (Workspace sidebar → right-click the repo → **Copy path**). That cell installs **`faiss-cpu`** (and related RAG deps) into the notebook kernel, then `pip install -e ...[rag,rag_embed]`, so both `import faiss` and `import nyaya_dhwani` work (editable extras alone are unreliable on some Free Edition runtimes).
